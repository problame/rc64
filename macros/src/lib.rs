...
impl syn::parse::Parse for Mi {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        let addr_modes = syn::punctuated::Punctuated::<syn::Path, syn::Token![,]>::parse_terminated(input)?.iter().cloned().collect();
        ...
        fn visit_expr_match_mut(&mut self, m: &mut syn::ExprMatch) {
            ...
            for (idx, arm) in m.arms.iter_mut().enumerate() {
                if let syn::Pat::Macro(syn::PatMacro { mac, .. }) = &arm.pat {
                    let segs = mac.path.segments.iter().map(|seg| seg.ident.to_string()).collect::<Vec<_>>();

                    if segs.len() == 1 && segs[0] == "mi" {
                        // parse mi!
                        let mi: Mi = syn::parse2(mac.tokens.clone()).expect("parse mi! args");
                        let v = &mi.variant;
                        let patterns_expanded = mi.addr_modes.iter().fold(proc_macro2::TokenStream::new(), |mut a, m| {
                            if !a.is_empty() {
                                a.extend(quote! {|});
                            }
            ...
            for (idx, out) in mods {
                ...
                m.arms[idx].pat = syn::Pat::Or(syn::PatOr { attrs: Vec::new(), leading_vert: None, cases: ors });
            }
        }
    }
